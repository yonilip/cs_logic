...
def _evaluate_constant(s):
    if s == 'T':
        ...
        raise ValueError(f"Given string isnt boolean. It is {s}")
...
def evaluate(formula, model):
    ...
    if formula.is_binary_formula():
        if formula.root == '&':
            ...
            raise Exception(f"Invalid binary formula\n"
                            f"formula parts:"
                            f"root: {formula.root} , first: {formula.first} , second: {formula.second}")
    else:
        raise Exception(f"Invalid formula. Formula is {formula}")
...
def all_models(variables):
    """ Return an iterator over all possible models over the variables in the
        ...
        precedes True """
    # Task 2.2
    # yield {var: False for var in variables}
    # for i in reversed(range(len(variables))):
    #     yield {var: variables.index(var) > i for var in variables}
    # for i in range(len(variables)):
    #     yield {var: variables.index(var) > i + 1 for var in variables}
...
def _print_header(sorted_vars, formula_as_str):
    ...
    for v in sorted_vars:
        s += f'| {v} '
    formula_format = f'| {formula_as_str} |'
...
def _print_inner_table(sorted_vars, formula, formula_as_str):
    ...
    for model in all_models(sorted_vars):
        ...
        s += f'| {evaluated_formula}' + suffix_str
        print(s)
